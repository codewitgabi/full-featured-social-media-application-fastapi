...
def test_create_comment_invalid_content(
    ...
    response = client.post(endpoint, headers={"authorization": f"Bearer {access_token}"}, json={})
...
def test_create_comment_unautthenticated_user(
        mock_db_session: Session):
...
def test_create_comment_post_not_found(
    ...
    response = client.post(endpoint, headers={"authorization": f"Bearer {access_token}"}, json={"comment": "Test comment"})
