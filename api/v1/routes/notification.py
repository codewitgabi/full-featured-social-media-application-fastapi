...
async def sse_endpoint(user: User = Depends(user_service.get_current_user)):
    return StreamingResponse(notification_service.event_generator(user.id), media_type="text/event_stream")


@notifications.get("")
async def user_notifications(user: User = Depends(user_service.get_current_user), db: Session = Depends(get_db)):
